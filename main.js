var socket = io.connect(
    "http://" + document.domain + ":" + location.port + "/car"
);
...
retrieveCarState(function(result) {
    ...
    $(document).ready(function() {
        ...
        $("#speed-group").hide();
        $("#speed-limit").hide();
        $("#control-group-classification").hide();
    ...
    if (sliderDir) {
        ...
        sliderDir.noUiSlider.on("update", function(
            values,
            handle,
            unencoded,
            isTap,
    ...
    if (sliderGas) {
        ...
        sliderGas.noUiSlider.on("update", function(
            values,
            handle,
            unencoded,
            isTap,
...
$("[data-mode]").click(function(event) {
    ...
    } else if (mode == "classtraining") {
        $("#model-group").hide();
        $("#speed-group").hide();
        $("#control-group").show();
        $("#control-group-classification").show()
        $("#speed-limit").show();
        $("#starter").show();
        $("#starter").prop("disabled", false);
...
$("[data-classification]").click(function (event) {
    event.preventDefault();
    ...
    $("[data-classification]").removeClass("btn-primary");
    $(this).toggleClass("btn-outline-primary btn-primary");

    socket.emit("classification_update", classification);
...
socket.on("max_speed_update_callback", function(data) {
    $("#maxSpeed").text(
        "Max speed limit: " + Math.round(data.speed * 100) + "%"
    );
});
...
socket.on("picture_stream", function(data) {
    ...
    if (data.image) {
        $("#stream_image").attr(
            "xlink:href",
            "data:image/jpeg;base64," + data.buffer
        );
